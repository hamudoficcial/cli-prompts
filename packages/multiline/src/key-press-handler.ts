...
const isPrintable = (key: { ctrl: boolean; meta: boolean; sequence?: string }): boolean => {
...
function deleteWord(state: State) {
  ...
  if (textBefore?.length === 0 && state.cursor.y > 0) {
    ...
    state.value[state.cursor.y] =
      (textBefore?.substring(0, newCursorX) ?? '') + (line?.substring(state.cursor.x) ?? '');
      ...
      case 'enter':
        {
          ...
          state.value[state.cursor.y] = currentLine.substring(0, state.cursor.x);
      ...
      case 'delete':
        {
          ...
          if (state.cursor.x < cl.length) {
            state.value[state.cursor.y] =
              cl.substring(0, state.cursor.x) + cl.substring(state.cursor.x + 1);
      ...
      default:
        if (isPrintable(key)) {
          ...
          if (state.maxLength && currentLength >= state.maxLength) {
            ...
            const cl = state.value[state.cursor.y] ?? '';
            state.value[state.cursor.y] =
              cl.substring(0, state.cursor.x) + key.sequence + cl.substring(state.cursor.x);
