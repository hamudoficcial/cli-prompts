...
export class InputParser extends EventEmitter {
  ...
  private tryMatchSequence(): boolean {
    const sequences = Object.keys(KEY_SEQUENCES).sort(
    ...
    if (this.buffer.startsWith("\x1b")) {
      ...
      if (this.buffer.length > 1) {
        ...
        this.emit("key", { sequence: this.buffer.slice(0, 2), name: nextChar, shift: false, ctrl: false, meta: true });
        this.buffer = this.buffer.slice(2);
      } else {
        this.emit("key", { sequence: "\x1b", name: "escape", shift: false, ctrl: false, meta: false });
        this.buffer = this.buffer.slice(1);
      }
      return true;
    }
