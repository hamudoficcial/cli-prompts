...
export function render(state: State): string {
  ...
  const prefix = state.spinner
    ? style.colors.prompt(state.spinner.frames[state.spinner.currentFrame] ?? '')
  ...
  if (isInputEmpty && state.placeholder) {
    const placeholderLines = state.placeholder
      ...
      .map(
        line =>
          `${style.colors.inputPrefix(style.prefixes.input)} ${style.colors.placeholder(line)}`
      );
  ...
  if (state.error) {
    lines.push(`${style.colors.error(style.prefixes.error)} ${style.colors.error(state.error)}`);
  }
...
export function finalize(state: State): string {
  ...
  let output = '';
  output += escapes.cursorRestorePosition + escapes.eraseDown;
  const promptLine = `${style.colors.success(style.prefixes.success)} ${state.prompt}`;
